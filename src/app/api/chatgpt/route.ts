...
export async function POST(req: Request) {
  ...
  if (!success) {
    ...
    return new NextResponse(JSON.stringify({ Message: "Hi, the companions can't talk this fast." }), {
      status: 429,
      headers: {
  ...
  if (!clerkUserId || !!!(await clerk.users.getUser(clerkUserId))) {
    ...
    return new NextResponse(JSON.stringify({ Message: "User not authorized" }), {
      status: 401,
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const companionFilePath = path.join(process.cwd(), "companions", companionFileName);
  const data = fs.readFileSync(companionFilePath, "utf8");
